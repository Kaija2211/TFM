...
- [x] Give every AI club a real transfer budget and annual wage bill. `ManagerClubFinance.ApplyAnnualWageBill` (the exact method the managed team's own season rollover already used) now runs for every club with a generated squad, not just the managed one; every club's budget is also seeded the moment its squad first exists, not only at rollover. Live-verified: Liverpool's own wage bill (£140.8m) deducted correctly from a real live season. AI clubs still can't spend this budget on anything - the transaction/decision layer (bid, sale, contract) is separate future work now unblocked by this foundation.
...
1. [x] Give AI clubs real Condition/injury-aware matchday rotation, a positional depth/need evaluator, a read-only transfer-target search, and a real budget/wage-bill foundation (`ManagerAiSquadRotation`, `ManagerAiSquadDepthEvaluator`, `ManagerAiTransferTargetSearch`, `ManagerClubFinance.ApplyAnnualWageBill`) on top of the completed 30-player club squads. Actually signing a target - the transaction/decision layer - is the next slice.
