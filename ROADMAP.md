...
- [x] Identify needs and search for tactically appropriate targets. `ManagerAiTransferTargetSearch` finds and ranks genuine upgrades (position fit, quality improvement over the club's current best, age-aware suitability) for a club's weakest position across the wider generated world. Read-only - no budget check, no transaction.
- [x] Give every AI club a real transfer budget and annual wage bill. `ManagerClubFinance.ApplyAnnualWageBill` (the exact method the managed team's own season rollover already used) now runs for every club with a generated squad, not just the managed one; every club's budget is also seeded the moment its squad first exists, not only at rollover. Live-verified: Liverpool's own wage bill (£140.8m) deducted correctly from a real live season.
- [x] Let AI clubs actually complete transfers - the first slice of "replace sold, injured, declining and retiring players." `ManagerAiTransferExecutor` gives each AI club one attempt per season rollover to sign a genuine upgrade for its weakest position, if it can afford the fee and the selling club retains adequate cover afterward. AI-to-AI only (never the managed team's squad or market); only ever sells bench/reserve depth, never a current starter (avoids breaking `AgentTeam`'s formation-slot index alignment). Realistically infrequent by design - a 20-club full-league pass typically completes only 1-2 transfers, matching how rarely a real generated squad has a genuine gap (see `ManagerAiSquadDepthEvaluator`'s own findings). Live-verified across two consecutive season rollovers with zero errors. Daily/window-respecting cadence, star-player sales and richer selling-club reluctance remain future refinements.
- Replace sold, injured, declining and retiring players (star sales, contracts, loans - beyond the depth-only sales above).
...
1. [x] Give AI clubs real Condition/injury-aware matchday rotation, a positional depth/need evaluator, a transfer-target search, a real budget/wage-bill foundation, and a first working transaction layer (`ManagerAiSquadRotation`, `ManagerAiSquadDepthEvaluator`, `ManagerAiTransferTargetSearch`, `ManagerClubFinance.ApplyAnnualWageBill`, `ManagerAiTransferExecutor`) on top of the completed 30-player club squads. AI clubs can now identify a need, find a target, and complete an affordable, safe AI-to-AI transfer once per season. Star-player sales, a daily/window-respecting cadence, contracts and richer club decision-making remain.
...
3. Deepen AI recruitment: star-player sales, contracts, loans and long-career squad-health safeguards using trustworthy performance evidence.
