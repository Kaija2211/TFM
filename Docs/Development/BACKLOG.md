...
No known save-corruption or match-result P0 remains after the August 15 Sunderland regression pass. AI clubs now get real Condition/injury-aware matchday rotation, a positional depth/need evaluator, a transfer-target search, a real budget/wage-bill foundation, and a first working AI-to-AI transaction layer (August 16); the next ordered work is:

1. Deepen AI recruitment — star-player sales (currently only bench/reserve depth sells), a daily/window-respecting cadence (currently one attempt per club per season rollover), contracts, and richer club decision-making (personalities, succession planning, safeguards against hoarding/churn).
...
Third slice, same day: `ManagerAiTransferTargetSearch` finds and ranks genuine upgrades for a club's weakest position (fed directly from `ManagerAiSquadDepthEvaluator`'s output) across every other generated club's squad - position fit (>=0.80, the same tier the rest of squad selection treats as usable), a hard "must actually be better than the current best option" filter (never recommends a lateral move or downgrade), and an age-aware suitability score favouring players with more prime years left at equal quality. Deliberately read-only: no budget check, no transaction, because AI clubs had no finance/budget tracking of any kind at the time this was built - see the fourth slice, same day.

Fourth slice, same day: gave every AI club a real transfer budget and annual wage bill. `ManagerClubFinance.ApplyAnnualWageBill` was extracted from the managed team's own season-rollover logic (previously `ManagerPrototypeController.HubAndSeason.cs`'s `DeductManagedTeamWageBill`, hardcoded to the managed team only) into a real, independently-testable method on `ManagerClubFinance` itself, then wired to run for every club with a generated squad at season rollover (silent for AI clubs - no Inbox spam, matching the same human-notification principle established for AI Condition/injuries), plus budget seeding the moment any club's squad first exists rather than waiting for the first rollover. Live-verified against a real season: Liverpool's own wage bill (£140.8m) deducted correctly, all ~20 clubs' rollovers ran with zero errors.

Fifth slice, same day: `ManagerAiTransferExecutor` completes the chain - the first AI-club work that actually finishes a transfer. Once per season rollover, each AI club with a genuine need (reusing the depth evaluator's own NeedScore, calibrated down from an untested guess of 10 to 0.5 after that service's own full-league audit pass completed zero transfers at 10 - the real observed NeedScore distribution across generated squads is usually exactly 0 with only occasional small positive spikes) tries its ranked target list in order until one is both affordable (`ManagerClubFinance.GetMarketValue`) and safe to buy from (the selling club must retain at least one other adequately-fit player at that position afterward). Deliberately AI-to-AI only - the managed team is never a buyer, seller, or target - and only ever sells bench/reserve depth, never a current starter, since `AgentTeam.RemovePlayer` shrinks `StartingEleven` in place and would break the formation-slot index alignment `ChangeFormation`/`ManagerAiSquadRotation` rely on. A full 20-club league pass typically completes only 1-2 transfers in total - realistically infrequent given how rarely a real generated squad has an actionable gap, not a bug. Live-verified across two consecutive season rollovers with zero errors.
...
- [x] transfer target identification based on actual weaknesses and age profile — third slice above (`ManagerAiTransferTargetSearch`). Value, wages and club stature as *targeting* inputs (rather than just budget existing at all) remain refinements;
- [x] every AI club has a real transfer budget and pays its own annual wages — fourth slice above (`ManagerClubFinance.ApplyAnnualWageBill`);
- [x] rational bid and replacement decisions, refusing structurally damaging sales — fifth slice above (`ManagerAiTransferExecutor`), for bench/reserve depth only. Star-player sales, loans and contracts remain;
