...
No known save-corruption or match-result P0 remains after the August 15 Sunderland regression pass. AI clubs now get real Condition/injury-aware matchday rotation, a positional depth/need evaluator, a read-only transfer-target search, and a real budget/wage-bill foundation (August 16); the next ordered work is:

1. Actual AI recruitment — bid/sale/contract decisions and squad changes acting on `ManagerAiTransferTargetSearch`'s output using each club's now-real budget. This is the transaction/decision layer; the finance foundation it needed is done.
...
- [x] every AI club has a real transfer budget and pays its own annual wages — fourth slice above (`ManagerClubFinance.ApplyAnnualWageBill`). Actually spending that budget (bids, contracts) is the next slice;
